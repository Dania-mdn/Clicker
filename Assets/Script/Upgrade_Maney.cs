...
public class Upgrade_Maney : MonoBehaviour
{
    public Image sprite;
    ...
    private void Start()
    {
        ...
        {
            iManey = PlayerPrefs.GetInt("Tap_maxValue");
            PlayerPrefs.SetFloat("Tap_maxValue", tap.Max_ofline + (iManey * 0.01f));
            cistern.text = "+ " + iManey.ToString("0") + "%";
        }
        else
        {
            iManey = Maney;
            tap.Max_ofline = tap.Max_ofline + (iManey * 0.01f);
            cistern.text = "+ " + iManey.ToString("0") + "%";
        }
    }
    private void Update()
    {
        ...
        if (i < price.Length)
        {
            if (price[i] < PlayerPrefs.GetFloat("Money_box"))
            {
                ...
                cistern.text = "+" + (Maney / 1000).ToString("0") + "%";
                if (is_dostup == false)
                {
    ...
    public void income()
    {
        if (i < price.Length && price[i] < PlayerPrefs.GetFloat("Money_box"))
        {
            ...
            tap.Max_ofline = tap.Max_ofline - (0.01f * iManey);
            iManey = iManey + Maney;
            tap.Max_ofline = tap.Max_ofline + (0.01f * iManey);
            PlayerPrefs.SetFloat("Maney", iManey);
            cistern.text = "+" + (Maney / 1000).ToString("0") + "%";

            //сохранить изменения
            PlayerPrefs.SetFloat("Max_ofline", tap.Max_ofline);
